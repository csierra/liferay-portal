...
public class PasswordMetaLocalServiceImpl
	 ...
	 * is the oldest entry, then also delete all PasswordHashProviders that
	...
	public PasswordMeta deletePasswordMeta(PasswordMeta passwordMeta) {
		...
		int olderMetas = _getPasswordMetasCountBeforeDate(createDate);

		if (olderMetas < 1) {
			_passwordHashProviderLocalService.
				deletePasswordHashProvidersBeforeDate(createDate);
		}
	...
	private int _getPasswordMetasCountBeforeDate(Date date) {
		return passwordMetaPersistence.countByLtCreateDate(date);
	}
