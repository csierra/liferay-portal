...
public class OAuth2ApplicationScopeAliasesLocalServiceImpl
	...
	public OAuth2ApplicationScopeAliases addOAuth2ApplicationScopeAliases(
		...
		for (OAuth2ApplicationScopes.ApplicationScopes applicationScope :
				oAuth2ApplicationScopes.getApplicationScopes()) {

			String applicationName = applicationScope.getApplicationName();

			for (OAuth2ApplicationScopes.ApplicationScope scope :
					applicationScope.getApplicationScopes()) {

				String scopeAlias = scope.getScopeAlias();
				String scopeName = scope.getScopeName();

				ArrayList<String> scopeAliasesList = new ArrayList<>();

				liferayOAuth2ScopesScopeAliases.put(
					_scopeLocator.getLiferayOAuth2Scope(
						companyId, applicationName, scopeName),
					scopeAliasesList);
