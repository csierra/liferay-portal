...
package com.liferay.portal.remote.jaxrs.utils.pagination;
...
public class PageContainerMessageBodyWriter
	...
	public void writeTo(
		...
		requestUriBuilder = requestUriBuilder.replaceQueryParam(
			"per_page", pageContainer.getItemsPerPage());

		if (pageContainer.hasNext()) {
			UriBuilder clonedRequestUtiBuilder = requestUriBuilder.clone();

			linkHeader.add(
				_link(
					clonedRequestUtiBuilder.replaceQueryParam(
						"page", currentPage + 1),
					"next"));
		...
		if (pageContainer.hasPrevious()) {
			UriBuilder clonedRequestUriBuilder = requestUriBuilder.clone();

			linkHeader.add(
				_link(
					clonedRequestUriBuilder.replaceQueryParam(
						"page", currentPage - 1),
					"prev"));

			linkHeader.add(
				_link(
					clonedRequestUriBuilder.replaceQueryParam("page", 1),
					"first"));
		}
	...
	private String _link(UriBuilder uriBuilder, String rel) {
		...
		return "<link href=\"" + uriString + "\" rel=\"" + rel + "\" />";
	}
