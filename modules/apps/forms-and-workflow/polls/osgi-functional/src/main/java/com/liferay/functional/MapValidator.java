...
public class MapValidator<T>
	...
	@Override
	public <S> MapValidator<S> map(Function1<T, S> g) {
	...
	@Override
	public <S> MapValidator<S> bind(
		Function1<T, Monad<MapValidator<?>, S>> fun) {

		return new MapValidator<>(
			h -> (Validation<S>) _f.apply(h).bind(fun.andThen((Function<Monad<MapValidator<?>, S>, Validation<S>>) mv -> ((MapValidator<S>)mv)._f.apply(h))));
	}
	...
	public static void main(String[] args) {
		...
		MapValidator<Integer> day = month.bind(
			m -> {
