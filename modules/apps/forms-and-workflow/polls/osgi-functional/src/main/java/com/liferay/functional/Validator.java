...
public interface Validator<T, R> extends Functor<R>{
	...
	default Validator<T, R> and(Validator<T, R> other) {
		return input -> (Validation<R>)validate(input).bind(
			o -> other.validate(input));
	}

	default <S> Validator<T, S> map(Function1<R, S> fun) {
	...
	default <S> Validator<T, S> compose(Validator<R, S> validator) {
		return input -> (Validation<S>) validate(input).bind(
			validator::validate);
	}
	...
	public static String testMethod(Consumer<User> consumer) {
		return "hola";
	}
...
//		System.out.println(result);
		...
		Validator<Optional<String>, Integer> safeMonth =
			isThereAnInteger.compose(
					lowerThan(13).and(greaterThan(0))).map(x -> x - 1);

		Validator<Optional<String>, Integer> safeYear =
			isThereAnInteger.compose(
					lowerThan(10000).and(greaterThan(1900))).map(x -> x - 1900);
