...
public class CXFEndpointPublisher {
	...
	private static class ServicesRegistrator {
		...
		protected void start() {
			...
			Object initPortalAuthorizationContextObject = _properties.get(
				"init.portal.authorization.context");

			boolean initPortalAuthorizationContext = true;

			if (initPortalAuthorizationContextObject != null) {
				initPortalAuthorizationContext = Boolean.parseBoolean(
					initPortalAuthorizationContextObject.toString());
			}

			if (initPortalAuthorizationContext) {
				properties = new Hashtable<>();
