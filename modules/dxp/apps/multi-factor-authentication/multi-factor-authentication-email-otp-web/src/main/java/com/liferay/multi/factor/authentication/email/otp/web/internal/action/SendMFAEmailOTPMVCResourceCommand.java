...
import com.liferay.multi.factor.authentication.email.otp.web.internal.constants.MFAPortletKeys;
...
public class SendMFAEmailOTPMVCResourceCommand implements MVCResourceCommand {
	...
	public boolean serveResource(
		...
		try {
			...
			if (!_isResendTimedOut(mfaEmailOTPConfiguration, httpSession)) {
				return false;
			}
			...
			httpSession.setAttribute(
				MFAEmailOTPWebKeys.MFA_EMAIL_OTP, generatedOTP);
	...
	private boolean _isResendTimedOut(
		...
		long otpSetAt = GetterUtil.getLong(
			session.getAttribute(MFAEmailOTPWebKeys.MFA_EMAIL_OTP_SET_AT_TIME),
			Long.MIN_VALUE);
		...
		if (_log.isDebugEnabled()) {
			_log.debug(
				StringBundler.concat(
					"OTP email sent to user id ", toUser.getUserId(),
